...
- `flake8 .` -> 0 errors. Confirmed this cycle.
- `python3 -m tests.test_parsers_offline` -> "All checks passed." Confirmed
  this cycle. Last visible section: "write_csv width/height non-finite/
  non-positive guards" all OK.
- C59-01 fix verified working: synthetic SpecDerived with `size=(0.0, 0.0)`,
  `(-1, -1)`, `(inf, 24)`, `(35.9, nan)` etc. all produce empty width/height
  cells in CSV output.
